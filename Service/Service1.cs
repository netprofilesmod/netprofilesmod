...
namespace Service
{
    ...
    public sealed class ServerNamedPipe : IDisposable
    {
        ...
        private void PipeListener()
        {
            ...
            try
            {
                Listen = Service1.PipeManager.Listen;
                ...
                while (Listen)
                {
                    ...
                    if (request.Trim() != "")
                    {
                        PipeConnection.Write(Service1.PipeManager.HandleRequest(request));
        ...
        internal void Close()
        {
            CheckIfDisposed();
            this.Listen = false;
            Service1.PipeManager.RemoveServerChannel(this.PipeConnection.NativeHandle);
            this.Dispose();
        }
        internal void Start()
        {
